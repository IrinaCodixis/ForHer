...
<div class="page">
		...
		<div class="body">
			...
			<?php 
$sql = "SELECT * FROM products"; //query to display all products
$result = @mysql_query($sql);

if (!$result)  //check if query does not work, return error
{    
...
{ 
	echo " <table style = 'width: 70%'>"; //else, display table with the result

	while ($query_row = @mysql_fetch_array ($result)) //get each row one by one
	{	
		$id = $query_row["id"];
		$items = array($query_row['name']);
		$prices = array($query_row['price']);
